...
<template>
  <div class="topTool">
    <el-input v-model="sname" placeholder="请输入非Vip姓名搜索" @input="handleSearchName" :prefix-icon="Search">
    </el-input>
...
onMounted(() => {
  axios.post('/admin/getAllNotVipUser')
    .then(response => {
...
function handleDelete(row) {// 处理删除按钮点击事件
  ...
  ElMessageBox.confirm(`是否删除 ID 为 ${row.id} 的用户？`, "删除用户", {
    ...
    console.log(`${row.id}`)
    axios.delete('/user/delUser' + '?id=' + row.id)
      .then(response => {
