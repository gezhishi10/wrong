...
<template>
    <div class="topTool">
      <el-input v-model="typename" placeholder="请输入房间类型搜索" @input="handleSearchName" :prefix-icon="Search">
      </el-input>
    ...
    <div style="padding: 20px;">
      ...
      <el-table :data="displayedItems" :header-cell-style="{ background: '#f6f9fa' }">
        ...
        <el-table-column prop="num" label="可住人数"></el-table-column>

        <el-table-column label="图片" min-width="180">
  <template #default="{ row }">
    <el-image :src="axios.defaults.baseURL+row.img" style="width: 80px; height: 80px" fit="cover"></el-image>
  </template>
</el-table-column>
  ...
  //这个是编辑的对话框 -->
    ...
    <el-dialog v-model="dialogFormVisible" title="编辑">
      <el-form :model="tableform">
        ...
        <el-form-item label="可住人数" :label-width="100" style="width: 20;">
            <el-input-number v-model="tableform.num" :min="1" :max="10"  />
        </el-form-item>

        <el-form-item label="图片" :label-width="100" style="width: 20;">
        <el-select  v-model="tableform.img">
            <el-option
...
<el-form-item label="图片效果" :label-width="100" style="width: 20;">
  <img :src="axios.defaults.baseURL+tableform.img" v-if="tableform.img" style="width: 80px; height: 80px">
        </el-form-item>
...
<!-- <el-form-item label="图片" :label-width="100" style="width: 20;">
    <el-select v-model="tableform.img">
      <el-option v-for="item in options" :key="item.id" :label="item.img" :value="item.img" />
    </el-select>
    <img :src="tableform.img" v-if="tableform.img" style="max-width: 100%; margin-top: 10px;">
</el-form-item> -->
  ...
  <el-dialog v-model="adddialogVisible" title="添加">
    <el-form :model="tableform">
        ...
        <el-form-item label="创建人" :label-width="100" style="width: 20;">
          <el-input v-model="tableform.createname" placeholder="创建人" />
        </el-form-item>
        <el-form-item label="可住人数" :label-width="100" style="width: 20;">
            <el-input-number v-model="tableform.num" :min="1" :max="10"  />
...
//   {
//     name: 'food.jpeg',
//     url: axios.defaults.baseURL+'/img/PremierSuite.jpg'
//   }
// ])
...
// }
  ...
  function saveData(){
    console.log("保存数据："+tableform.value)
    ...
    axios.post('/type/addRoomType', type, {
      headers: {
  ...
  function saveEdit() {
    ...
    let type = JSON.stringify(tableform.value)
    console.log("保存type:" + type)
  
    axios.post('/type/updataTypeInfo', type, {
      headers: {
        'Content-Type': 'application/json'
      }
    })
      .then(response => {
        ...
        if (targetItem) {
          ...
          targetItem.createtime = tableform.value.createtime;
          targetItem.createname = tableform.value.createname;
          targetItem.num = tableform.value.num;
          targetItem.img = tableform.value.img;
  ...
  function handleDelete(row) {// 处理删除按钮点击事件
    ...
    ElMessageBox.confirm(`是否删除 ID 为 ${row.id} 的房间类型吗？`, "删除类别", {
      ...
      console.log(`${row.id}`)
      axios.delete('/type/delRoomType' + '?id=' + row.id)
        .then(response => {
  ...
  const handleSearchName = (val) => {
    if (val.length > 0) { // 搜索框有输入
      ...
      displayedItems.value = typeData.value.filter(item => item.typename.match(val))
    } else { // 搜索框无输入
