...
<template>
    ...
    <div style="padding: 20px;">
      ...
      <el-table :data="displayedItems" :header-cell-style="{ background: '#f6f9fa' }">
        ...
        <el-table-column prop="roomid" label="房间号码"></el-table-column>
        ...
        <el-table-column prop="createname" label="用户名"></el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" title="编辑">
      <el-form :model="tableform">
        <el-form-item label="房间编号" :label-width="100" style="width: 20;">
          <el-input v-model="tableform.roomid" placeholder="房间编号" />
        </el-form-item>
        <el-form-item label="评论时间" :label-width="100">
          <el-input v-model="tableform.commenttime" placeholder="评论时间" />
        </el-form-item>
        <el-form-item label="用户id" :label-width="100">
          <el-input v-model="tableform.userid" placeholder="用户编号" />
        </el-form-item>
        <el-form-item label="评论内容" :label-width="100">
          <el-input v-model="tableform.content" placeholder="评论内容" />
        </el-form-item>
        <el-form-item label="用户名" :label-width="100">
          <el-input v-model="tableform.createname" placeholder="用户名" />
      </el-form-item>
      </el-form>
  ...
  onMounted(() => {
    axios.post('/roomComment/getAllRoomComment')
      .then(response => {
  ...
  function saveData(){
    ...
    axios.post('/roomComment/addRoomComment', user, {
      headers: {
  ...
  function saveEdit() {
    ...
    axios.post('/roomComment/updataRoomComment', user, {
      headers: {
        'Content-Type': 'application/json'
      }
    })
      .then(response => {
        ...
        if (targetItem) {
          ...
          targetItem.commenttime=tableform.value.commenttime;
          targetItem.userid=tableform.value.userid;
          targetItem.content=tableform.value.content;
  ...
  function handleDelete(row) {// 处理删除按钮点击事件
    ...
    ElMessageBox.confirm(`是否删除 ID 为 ${row.id} 的用户？`, "删除用户", {
      ...
      console.log(`${row.id}`)
      axios.delete('/roomComment/delRoomComment' + '?id=' + row.id)
        .then(response => {
  ...
  const handleSearchName = (val) => {
    console.log("筛选器的;"+val)
    if (val.length > 0) { // 搜索框有输入
      // 根据姓名进行过滤操作
      // displayedItems.value = userData.value.filter(item => item.roomid.match(val))
      displayedItems.value = itemsCopy.filter(item => item.roomtype && item.roomtype.toString().match(val));
    } else { // 搜索框无输入
